...
function replaceRules(flag, data) {
    if (flag == 'js') {
        ...
        // data = data.replace(/([\s]+)(data\:)/g, '$1data() {\n  return');
        // data = data.replace(/([\s]+)(data\:)[\s]?(\{([^\}]+)\})/g, '$1data() {\n return$3');
    ...
    //     data = data.replace(/("|')(\{\{)([^{]*)(\}\})("|')/g, '$1$3$5')
    //         // view 转div
    //     data = data.replace(/(\<)view/g, '$1div')
    //     data = data.replace(/(\<\/)view/g, '$1div')
    //         // image 转 img
    //     data = data.replace(/(\<)image/g, '$1img')
    //     data = data.replace(/(\<\/)image\>/g, '')
    //         // text 转 span
    //     data = data.replace(/(\<)text/g, '$1span')
    //     data = data.replace(/(\<\/)text/g, '$1span')
