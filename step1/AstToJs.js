...
class AstToJs {
    ...
    init(ast) {
        ...
        ast.program.body.forEach((v, i) => {
            ast.program.body[i] = this.lookInPage(v)
        })
        return ast;
    }
    ...
    convertPageDataProperty(astPage) {
        // 匿名函数表达式
        ...
        if (ast.type === PARAMS.EXPRESSION_STATEMENT) {
            ...
            if (ast.expression.type === PARAMS.CALL_EXPRESSION) {
                ...
                if (ast.expression.callee.name === WEAPP.PAGE) {
                        ...
                        argumentList.properties.forEach((propertyItem, k) => {
                            ...
                            if (propertyItem.type == PROPERTY.PROPERTY) {
                                ...
                                if (propertyItem.key.name == WEAPP.DATA) {
                                    ...
                                    const newData = this.convertPageDataProperty(astPageDataNode)
                    ...
                    ast = this.convertPageMethod(ast);
                                ...
                                if (propertyItem.key.name == WEAPP.DATA) {
                                    ...
                                    if (propertyItem.value.properties.length) {
                                        const {
                                            index,
                                            item
                                        } = this.convertLifeTimes(ast, propertyItem);
                                        // 记录需要删除的属性索引
                                    ...
                                    if (propertyItem.value.properties.length) {
                                        const { watchArr } = this.convertPropertiesMethods(propertyItem);
                    ...
                    ast = this.convertComponentMethod(ast);
                }
            }
        }
        return ast;
    ...
    convertPropertiesMethods(propertiesObj) {
    ...
    convertLifetimesMethods(lifeTimesArr) {
        let newLifeTimesArr = [];
    ...
    convertLifeTimes(ast, lifeTimesItem) {
        const lifeTimesArr = lifeTimesItem.value.properties;
        lifeTimesArr.forEach(v => {
            v = objectExpression([v])
        });
        const afterConvertLifetimes = this.convertLifetimesMethods(lifeTimesArr);
